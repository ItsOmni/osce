...
export default function OSCEElearning() {
  ...
  // >>> Improved, styled certificate generator <<<
  const downloadCert = () => {
    ...
    // Helpers
    const center = (text: string, y: number, size = 12, font: "normal"|"bold"|"italic"|"bolditalic" = "normal", color = {r:20,g:24,b:31}) => {
    ...
    doc.roundedRect(m, m, W - 2*m, H - 2*m, 12, 12, "S");

    // Soft watermark (simulated with very light colour)
    doc.setTextColor(235, 238, 241);
    doc.setFont("times", "bolditalic");
    doc.setFontSize(96);
    const wm = "OSCE";
    doc.text(wm, W/2 - doc.getTextWidth(wm)/2, H/2 + 24, { angle: -20 });

    // Title block
    center("Certificate of Completion", 140, 28, "bold", {r:20,g:24,b:31});
    center("This certifies that", 180, 14, "normal");

    // Name line
    const name = (learnerName && learnerName.trim()) ? learnerName.trim() : "[Your name]";
    doc.setDrawColor(160);
    doc.line(W*0.18, 210, W*0.82, 210);
    center(name, 205, 24, "bold");

    // Body copy (wrapped)
    doc.setFont("times", "normal");
    doc.setFontSize(14);
    doc.setTextColor(20, 24, 31);
    const body = `has completed the “An Introduction to OSCEs” interactive e-learning led by Dr Hussain Hilali.`;
    const wrapped = doc.splitTextToSize(body, W * 0.68);
    doc.text(wrapped, W*0.16, 240);

    // Date + instructor area
    ...
    // Date
    doc.text("Date:", W*0.18, H - 150);
    doc.line(W*0.18 + 28, H - 152, W*0.35, H - 152);
    doc.text(dateStr, W*0.18 + 30, H - 155);

    // Instructor
    doc.text("Instructor:", W*0.55, H - 150);
    doc.line(W*0.55 + 56, H - 152, W*0.82, H - 152);
    doc.text("Dr Hussain Hilali", W*0.55 + 58, H - 155);

    // Footer note
    doc.setFont("times", "italic");
    doc.setFontSize(10);
    doc.setTextColor(90);
    const footer = "This is a self-certified completion record for personal development / portfolio use.";
    const fwrap = doc.splitTextToSize(footer, W - 2*m);
    doc.text(fwrap, m, H - 70);

    // Save
    const safeName = name.replace(/[^\w\s-]/g, "").replace(/\s+/g, "_");
    doc.save(`OSCE_Introduction_Certificate_${safeName || "Student"}.pdf`);
  };
          ...
          <SectionCard title="Certificate" subtitle="Unlocks when you finish" complete={eligible}>
            {eligible ? (
              ...
              <div className="p-4 rounded-2xl bg-yellow-50 border border-yellow-200">
                ...
                <p className="text-sm text-slate-700">Complete all sections and pass the 5-question check to unlock your certificate.</p>
              </div>
            )}
          </SectionCard>
        </div>
